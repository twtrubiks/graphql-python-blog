...
1. **階段一：包裝現有 REST API**
   ```javascript
   // GraphQL Resolver 內部調用 REST
   const resolvers = {
     Query: {
       user: (_, { id }) => fetch(`/api/users/${id}`)
     }
   }
   ```
...
REST 和 GraphQL 都是優秀的 API 設計方案，沒有絕對的優劣，只有適不適合。REST 以其簡單性和成熟度仍然是許多專案的首選，而 GraphQL 則為複雜的資料需求提供了優雅的解決方案。
